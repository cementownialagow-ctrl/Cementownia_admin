...
def appointments():
    ...
    return render_template_string(dispatch_tpl, rows=rows, orders=orders, wzs=wzs, transports=transports, drivers=drivers, driver_busy=driver_busy, vehicles=vehicles, vehicle_busy=vehicle_busy, bays=bays, day=day, labels=STAGE_LABEL, capacity_notice=request.args.get("capacity_notice", ""), title="Zaplanuj dostawę", base_url=D["BASE_URL"], db_path=D["DB_PATH"])
...
<div class="flex"><h1>Zaplanuj dostawę</h1><a class="btn right" href="{{url_for('dispatch.queue',day=day)}}">Plan wydań</a></div>
