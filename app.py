...
def next_invoice_no(issue_date: str) -> str:
    """Return the next invoice number based only on invoices actually saved.

    Opening the invoice form must never reserve/consume a number. The UNIQUE
    constraint on invoices.invoice_no remains the final protection against a
    ...
    try:
        cur = c.cursor()
        current = cur.execute(
            """SELECT COALESCE(MAX(CAST(substr(invoice_no,6,instr(substr(invoice_no,6),'/')-1) AS INTEGER)),0)
               FROM invoices
               WHERE substr(issue_date,1,7)=?
                 AND invoice_no LIKE 'FVAT %/%/%'""",
            (f"{yyyy}-{mm}",)
        ).fetchone()[0]
        return f"FVAT {int(current or 0) + 1}/{mm}/{yyyy}"
    finally:
        c.close()
...
def order_fully_invoiced(cur, order_id: int) -> bool:
    ...
    return all(
        ...
        and float(done.get(int(row["id"]), 0.0)) + eps >= float(row["qty"] or 0)
        for row in rows
    )
...
def invoice_edit_items(invoice_id: int, invoice_row: dict):
    ...
    for it in items:
        item_id = int(it["id"])
        current_qty = float(current_qty_by_item.get(item_id, 0.0))
        ordered_qty = float(it.get("qty") or 0)
        invoiced_total = float(invoiced_by_item.get(item_id, 0.0))
...
def order_item_update(order_id, item_id):
    ...
    if INVENTORY_AUTOMATION_ENABLED and int(o["warehouse_issued"] or 0) == 1:
        c.close()
        return "ZamĂłwienie wydane z magazynu jest tylko do podglÄ…du", 400
    invoiced_qty = to_float(invoiced_qty_by_order_item_ids([item_id]).get(int(item_id)), 0.0)
    if qty < invoiced_qty:
        c.close()
...
def order_invoice(order_id):
    ...
    for it in items:
        ...
        ordered_qty = to_float(it.get("qty"), 0.0)
        done_qty = to_float(invoiced_by_item.get(int(it["id"])), 0.0)
        ...
        if wz_id:
            it["remaining_qty"] = min(it["remaining_qty"], to_float(wz_qty.get(int(it["id"])), 0.0))
    ...
    if request.method == "GET":
        ...
        if wz_id:
            ...
            wz_mismatch = (
                ...
                or any(abs(selected[item_id] - expected[item_id]) > eps for item_id in expected)
            )
            ...
            <table>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <input type="number" min="0" step="0.01" name="invoice_qty_{{ it['id'] }}" value="0" max="{{ it['remaining_qty'] }}" style="width:110px;" {% if it['remaining_qty'] <= 0 %}disabled{% endif %}>
                  </td>
...
def invoice_edit_admin(invoice_id):
    ...
    if request.method == "POST":
        ...
        if not data["invoice_no"]:
            ...
            old_order_ids = sorted({int(x.get("source_order_id") or x.get("order_id") or 0) for x in edit_items if to_float(x.get("current_invoice_qty"), 0.0) > 0})
            st, pc, city = split_address(data.get("buyer_address", ""))
            c = conn()
            cur = c.cursor()
            cur.execute("""
              ...
              <tbody>
                {% for it in edit_items %}
                  <tr>
                    ...
                    <td><b>{{ it.remaining_qty }}</b></td>
                    <td><input type="number" min="0" step="0.01" max="{{ it.remaining_qty }}" name="invoice_qty_{{ it.id }}" value="{{ it.current_invoice_qty }}" style="width:110px;"></td>
                    <td>{{ "%.2f"|format(it.net_price) }}</td>
                    <td>{{ "%.2f"|format(it.gross_price) }}</td>
                  </tr>
                {% endfor %}
