...
def init_db():
    ...
    if "delivery_method" not in order_cols:
        cur.execute("ALTER TABLE orders ADD COLUMN delivery_method TEXT NOT NULL DEFAULT 'delivery'")
...
def next_invoice_no(issue_date: str) -> str:
    ...
    c = conn()
    cur = c.cursor()
    # To jest wyłącznie podgląd następnego numeru. Samo otwarcie formularza
    # nie może rezerwować ani zużywać numeru faktury.
...
def remote_first_create_order(customer_id, customer_name, customer_address, customer_phone, customer_email, note, items, idempotency_key=None, delivery_date="", delivery_time="", delivery_method="delivery"):
    created_at = now_iso()
    ...
    try:
        cur = c.cursor()
        cur.execute(
            "INSERT INTO orders(id, order_no, customer_id, customer_name, customer_address, customer_phone, customer_email, delivery_date, delivery_time, delivery_method, status, note, created_at, idempotency_key) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET order_no=excluded.order_no, customer_id=excluded.customer_id, customer_name=excluded.customer_name, customer_address=excluded.customer_address, customer_phone=excluded.customer_phone, customer_email=excluded.customer_email, delivery_date=excluded.delivery_date, delivery_time=excluded.delivery_time, delivery_method=excluded.delivery_method, status=excluded.status, note=excluded.note, created_at=excluded.created_at, idempotency_key=excluded.idempotency_key",
            (order_id, order_no, customer_id if customer_id else None, customer_name, customer_address, customer_phone, customer_email, delivery_date or None, delivery_time or None, delivery_method, "new", note, created_at, idempotency_key)
        )
...
def invoiced_qty_by_order_item_ids(order_item_ids: list[int]):
    ...
    """, tuple(ids))
    for r in cur.fetchall():
        out[int(r["order_item_id"])] = float(r["qty"] or 0)
    c.close()
    return out
...
def order_fully_invoiced(cur, order_id: int) -> bool:
    ...
    cur.execute(f"""
      ...
      GROUP BY order_item_id
    """, tuple(item_ids))
    done = {int(r["order_item_id"]): float(r["qty"] or 0) for r in cur.fetchall()}
    return all(float(row["qty"] or 0) > 0 and float(done.get(int(row["id"]), 0)) + 1e-9 >= float(row["qty"] or 0) for row in rows)
...
def invoice_edit_items(invoice_id: int, invoice_row: dict):
    ...
    current_qty_by_item = {int(r["order_item_id"]): float(r["qty"] or 0) for r in current_alloc_rows}
    ...
    for it in items:
        item_id = int(it["id"])
        current_qty = float(current_qty_by_item.get(item_id, 0))
        ordered_qty = float(it.get("qty") or 0)
        invoiced_total = float(invoiced_by_item.get(item_id, 0))
...
<body>
  ...
  <div class="top">
    ...
    <div class="nav flex">
      ...
      <a class="nav-transports" href="{{ url_for('beton.transports') }}">Zaplanowane dostawy</a>
      <a href="{{ url_for('dispatch.appointments') }}">Zaplanuj dostawę</a>
...
def home():
    ...
    warehouse_role = session.get("role") == "warehouse"
    due_to_issue_today = int(cur.execute("""SELECT COUNT(*) AS n
      FROM dispatch_appointments a JOIN transports t ON t.id=a.transport_id
      WHERE a.planned_date=? AND a.status NOT IN ('cancelled','returned')
        AND t.status IN ('assigned','issued') AND t.deleted_at IS NULL""", (today_iso,)).fetchone()["n"] or 0)
    c.close()
    ...
    {% block content %}
      ...
      <div class="metrics">
        {% if warehouse_role %}<a class="metric" style="--soft:#fff5e5;--tone:#c57a10;text-decoration:none;color:inherit" href="{{ url_for('dispatch.appointments', day=today_iso) }}"><div class="icon">!</div><div><span>DO WYDANIA DZISIAJ</span><b>{{ due_to_issue_today }}</b><small>Przejdź do planu wydań</small></div></a>{% endif %}
      ...
      {% if not warehouse_role %}<div class="metrics" style="grid-template-columns:minmax(260px,1fr);max-width:360px">
    ...
    return render_template_string(tpl, title="Start", base_url=BASE_URL, db_path=DB_PATH,
                                  ...
                                  warehouse_role=warehouse_role,due_to_issue_today=due_to_issue_today,today_iso=today_iso)
...
def product_recipe(product_id):
    ...
    selected_version_items = []
    selected_version_id = to_int(request.args.get("version_id"), 0)
    if selected_version_id:
        selected_version = c.execute(
            "SELECT * FROM recipe_versions WHERE id=? AND product_id=?",
            (selected_version_id, product_id),
        ).fetchone()
        if selected_version:
            selected_version_items = c.execute("""SELECT rvi.qty_per_unit,rvi.unit,rvi.material_snapshot_json,
                    COALESCE(rm.name,'Usunięty surowiec') AS material_name
                FROM recipe_version_items rvi
                LEFT JOIN raw_materials rm ON rm.id=rvi.material_id
                WHERE rvi.recipe_version_id=? ORDER BY material_name""", (selected_version_id,)).fetchall()
    c.close()
    ...
    recipe_page = r'''{% extends "base.html" %}{% block content %}
    ...
    {% if selected_version %}<div class="card">
      <div class="flex"><div><h2>Podgląd wersji {{selected_version.version_no}}</h2><div class="muted">{{selected_version.name or 'Bez nazwy'}} · numer {{selected_version.recipe_no or '—'}} · obowiązuje od {{selected_version.valid_from or '—'}}</div></div><a class="btn right" href="{{url_for('product_recipe',product_id=product.id)}}">Zamknij podgląd</a></div>
      <div class="grid3" style="margin-top:18px">
        <div><b>Klasa betonu</b><br>{{selected_version.concrete_class or '—'}}</div><div><b>Konsystencja</b><br>{{selected_version.consistency or '—'}}</div><div><b>W/S</b><br>{{selected_version.water_cement_ratio if selected_version.water_cement_ratio is not none else '—'}}</div>
        <div><b>Klasa ekspozycji</b><br>{{selected_version.exposure_class or '—'}}</div><div><b>Maks. wymiar kruszywa</b><br>{{selected_version.max_aggregate_size or '—'}}</div><div><b>Klasa chlorków</b><br>{{selected_version.chloride_class or '—'}}</div>
        <div><b>Wytrzymałość charakterystyczna</b><br>{{selected_version.characteristic_strength or '—'}}</div><div><b>Norma / dokument</b><br>{{selected_version.reference_document or '—'}}</div><div><b>Rodzaj cementu</b><br>{{selected_version.cement_type or '—'}}</div>
      </div>
      <h3 style="margin-top:22px">Skład zapisany w tej wersji na 1 {{product.unit or 'm³'}}</h3>
      <table><thead><tr><th>Surowiec</th><th>Ilość</th></tr></thead><tbody>{% for x in selected_version_items %}<tr><td><b>{{x.material_name}}</b></td><td>{{x.qty_per_unit}} {{x.unit}}</td></tr>{% else %}<tr><td colspan="2" class="muted">Ta historyczna wersja nie zawiera zapisanego składu.</td></tr>{% endfor %}</tbody></table>
      <div class="grid3"><div><b>Domieszki</b><br>{{selected_version.admixtures or '—'}}</div><div><b>Włókna</b><br>{{selected_version.fibres or '—'}}</div><div><b>Inne dodatki</b><br>{{selected_version.other_additions or '—'}}</div></div>
      <p><b>Uwagi technologiczne:</b><br>{{selected_version.technology_notes or '—'}}</p>
    </div>{% endif %}

    <form method="post" class="card"><input type="hidden" name="form_action" value="save_version"><h2>Zapisz nową wersję receptury</h2>
      <div class="grid3"><div><label>Nazwa receptury</label><input name="recipe_name" required></div><div><label>Numer receptury</label><input name="recipe_no" required></div><div><label>Nowa wersja</label><input value="{{(versions[0].version_no+1) if versions else 1}}" readonly></div><div><label>Obowiązuje od</label><input type="date" name="valid_from" required></div></div>
      <h2>Parametry betonu</h2><div class="grid3"><div><label>Klasa betonu</label><input name="concrete_class" placeholder="C20/25"></div><div><label>Konsystencja</label><input name="consistency" placeholder="S3"></div><div><label>W/S</label><input type="number" step="0.01" name="water_cement_ratio" placeholder="0,55"></div><div><label>Klasa ekspozycji</label><input name="exposure_class" placeholder="XC2"></div><div><label>Maks. wymiar kruszywa</label><input name="max_aggregate_size" placeholder="16 mm"></div><div><label>Klasa chlorków</label><input name="chloride_class" placeholder="Cl 0,20"></div><div><label>Wytrzymałość charakterystyczna</label><input name="characteristic_strength"></div><div><label>Norma / dokument odniesienia</label><input name="reference_document" placeholder="PN-EN 206"></div><div><label>Rodzaj cementu</label><input name="cement_type"></div></div>
      <h2>Dodatki</h2><div class="grid3"><div><label>Domieszki</label><textarea name="admixtures"></textarea></div><div><label>Włókna</label><textarea name="fibres"></textarea></div><div><label>Inne dodatki</label><textarea name="other_additions"></textarea></div></div><label>Uwagi technologiczne</label><textarea name="technology_notes"></textarea><button class="btn primary" style="margin-top:12px">Zapisz jako nową wersję</button>
    </form>

    <div class="card"><h2>Wersje receptury</h2><table><thead><tr><th>Wersja</th><th>Numer</th><th>Nazwa</th><th>Od</th><th>Parametry</th><th></th></tr></thead><tbody>{% for v in versions %}<tr><td><b>{{v.version_no}}</b></td><td>{{v.recipe_no or '—'}}</td><td>{{v.name or '—'}}</td><td>{{v.valid_from or '—'}}</td><td>{{v.concrete_class or '—'}} · {{v.consistency or '—'}} · W/S {{v.water_cement_ratio if v.water_cement_ratio is not none else '—'}}</td><td><a class="btn" href="{{url_for('product_recipe',product_id=product.id,version_id=v.id)}}">Podgląd</a></td></tr>{% else %}<tr><td colspan="6">Brak zapisanej wersji.</td></tr>{% endfor %}</tbody></table></div>

    <div class="row"><div class="card"><h2>Skład</h2><form method="post"><label>Surowiec</label><input name="material_name" required><div class="row"><div><label>Ilość na 1 {{product.unit or 'm³'}}</label><input name="qty_per_unit" type="number" min="0.0001" step="0.0001" required></div><div><label>Jednostka</label><select name="unit"><option>kg</option><option>t</option><option value="m3">m³</option><option>l</option><option>szt.</option></select></div></div><button class="btn primary" style="margin-top:12px">Dodaj / zaktualizuj</button></form></div>
    <div class="card"><h2>Aktualny skład na 1 {{product.unit or 'm³'}}</h2><table><tbody>{% for x in recipe %}<tr><td><b>{{x.name}}</b></td><td>{{x.qty_per_unit}} {{x.unit}}</td><td><form method="post" action="{{url_for('product_recipe_item_delete',product_id=product.id,recipe_id=x.id)}}"><button class="btn danger">Usuń</button></form></td></tr>{% else %}<tr><td>Brak składu.</td></tr>{% endfor %}</tbody></table></div></div>
    {% endblock %}'''
    return render_template_string(recipe_page,product=product,recipe=recipe,versions=versions,selected_version=selected_version,selected_version_items=selected_version_items,error=error,title='Receptura',base_url=BASE_URL,db_path=DB_PATH)
    return render_template_string(r'''{% extends "base.html" %}{% block content %}
      <div class="flex"><h1>Receptura: {{product.name or product.sku}}</h1><a class="btn right" href="{{url_for('products')}}">Wróć do produktów</a></div>
      {% if error %}<div class="hint">{{error}}</div>{% endif %}
      <div class="row">
    ...
    {% block content %}
      ...
      <div class="card">
        <form method="post" action="{{ url_for('order_create') }}">
          ...
          <div class="row" style="margin-top:10px;">
            <div>
              <label class="muted small">Sposób realizacji</label>
            ...
            <div>
              <label class="muted small">Termin realizacji</label>
              <input name="delivery_date" type="date" required>
            </div>
          </div>
          <div class="row" style="margin-top:10px;">
            <div>
              <label class="muted small">Planowana godzina realizacji (opcjonalnie)</label>
              <input name="delivery_time" type="time">
            </div>
            <div class="muted" style="align-self:end; padding-bottom:14px;">Wybrany sposób realizacji zostanie wydrukowany na dokumencie WZ.</div>
          </div>
...
def order_create():
    ...
    delivery_method = norm(request.form.get("delivery_method")).lower()
    if delivery_method not in {"delivery", "pickup"}:
        return "Wybierz sposób realizacji: dostawa albo odbiór własny", 400
    if not delivery_date:
        return "Podaj termin realizacji", 400
...
def order_item_update(order_id, item_id):
    ...
    if INVENTORY_AUTOMATION_ENABLED and int(o["warehouse_issued"] or 0) == 1:
        c.close()
        return "ZamĂłwienie wydane z magazynu jest tylko do podglÄ…du", 400
    invoiced_qty = float(invoiced_qty_by_order_item_ids([item_id]).get(int(item_id)) or 0)
    if qty < invoiced_qty:
        c.close()
...
def order_invoice(order_id):
    ...
    for it in items:
        ...
        ordered_qty = float(it.get("qty") or 0)
        done_qty = float(invoiced_by_item.get(int(it["id"])) or 0)
        ...
        if wz_id:
            it["remaining_qty"] = min(it["remaining_qty"], float(wz_qty.get(int(it["id"]), 0)))
            ...
            <table>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <input type="number" min="0" step="0.01" name="invoice_qty_{{ it['id'] }}" value="{{ it['remaining_qty'] if wz_id else 0 }}" max="{{ it['remaining_qty'] }}" style="width:110px;" {% if it['remaining_qty'] <= 0 %}disabled{% endif %}>
                  </td>
    ...
    {% endblock %}
    """
    return render_template_string(tpl, title="Faktura", base_url=BASE_URL, db_path=DB_PATH, o=o, d=data, company=company, items=items, invoice_rows=invoice_rows, msg=msg, canonical_order_no=canonical_order_no, wz_id=wz_id)
...
def api_order_lookup():
    ...
    for it in items:
        ordered_qty = float(it.get("qty") or 0)
        invoiced_qty = float(invoiced_by_item.get(int(it["id"])) or 0)
        it["ordered_qty"] = ordered_qty
...
def invoice_edit_admin(invoice_id):
    ...
    if request.method == "POST":
        ...
        if not data["invoice_no"]:
            ...
            old_order_ids = sorted({int(x.get("source_order_id") or x.get("order_id") or 0) for x in edit_items if float(x.get("current_invoice_qty") or 0) > 0})
            st, pc, city = split_address(data.get("buyer_address", ""))
            c = conn()
            cur = c.cursor()
            cur.execute("""
